...
<template>
  <pv-dialog v-model:visible="clientDialog" modal header="New Client Registration" :style="{ width: '25rem' }">
    ...
    <form @submit.prevent="onSubmitItem">
      ...
      <pv-floatlabel class="flex flex-column items-center gap-2 my-5">
        <pv-inputnumber v-model="age" id="age" :useGrouping="false" class="flex-auto" autocomplete="off" required />
        <label for="age" class="flex align-items-center font-semibold w-24">Age:</label>
      </pv-floatlabel>
      <pv-floatlabel class="flex flex-column items-center gap-2 my-5">
        <pv-inputtext v-model="location" id="location" class="flex-auto" autocomplete="off" required />
        <label for="location" class="flex align-items-center font-semibold w-24">Location:</label>
      </pv-floatlabel>
