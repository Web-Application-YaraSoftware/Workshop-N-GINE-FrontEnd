...
function createUser() {
  accessService.signUp(credentials.value)
    .then(response => {
      toast.add({severity: 'success', summary: 'Success', detail: "Welcome!"});
      goToSignIn();
    })
    .catch( () => {
...
function createWorkshop() {
  workshopService.post({name: workshopName.value})
    .then(response => {
      credentials.value.workshopId = response.data.id;
      createUser();
    })
    .catch(() => {
      toast.add({severity: 'error', summary: 'Error', detail: "Invalid workshop name"});
...
function IsValid() {
  return username.value.length > 0 && password.value.length > 0 && workshopName.value.length > 0 && password.value === repeatPassword.value;
}
      ...
      <template #content>
        <div class="sections">
          <div class="section sign-up-section">
            <div class="content">
              <h1>Join Us and Transform Your Workshop</h1>
              <p>Experience seamless management and skyrocket your workshop's performance. Sign up today and take the first step toward success!</p>
              <router-link :to="{name: 'login'}">
                <pv-button label="Sign in" class="w-full"></pv-button>
              </router-link>
            </div>
          </div>
          <div class="section sign-in-section">
            <span class="pi pi-car icon"></span>
            <h1>Ready to Drive Forward?</h1>
            <form class="form" @submit.prevent="onSubmit">
              <pv-floatlabel class="w-full">
                <pv-inputtext class="w-full my-2" v-model="workshopName"/>
                <label for="email">Workshop</label>
              </pv-floatlabel>
              <pv-floatlabel class="w-full">
                <pv-inputtext class="w-full my-2" v-model="username"/>
                <label for="email">Username</label>
              </pv-floatlabel>
              <pv-floatlabel class="w-full">
                <pv-password class="w-full my-2" v-model="password" toggleMask fluid/>
                <label for="password">Password</label>
              </pv-floatlabel>
              <pv-floatlabel class="w-full">
                <pv-password class="w-full my-2" v-model="repeatPassword" toggleMask fluid/>
                <label for="password">Repeat password</label>
              </pv-floatlabel>
              <pv-button type="submit" label="Sign up" class="my-3 w-fit m-auto"></pv-button>
            </form>
          </div>
        </div>
      </template>
